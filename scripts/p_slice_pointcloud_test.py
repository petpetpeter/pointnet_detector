...
def uniform_slice_layer(pcl_data,number_of_layer = 100):
    ...
    for i in tqdm(range(number_of_layer)):
        ...
        if np.count_nonzero(layer_label == 1) > 0.05*len(layer_label):
            print(f"burr layer found at layer:{i}")
...
INPUT_DIR = "/home/peter/Documents/python_ws/PointNet_Custom_Object_Detection/test_input"
...
for file in os.listdir(INPUT_DIR):
    if file.endswith(".pcd"):
        ...
        #pc_data.visualize()
        list_of_burr_layers = uniform_slice_layer(pc_data,number_of_layer = 50)
        final_burr_layer_list = final_burr_layer_list + list_of_burr_layers
        #for layer in list_of_burr_layers:
        #   layer.visualize()
